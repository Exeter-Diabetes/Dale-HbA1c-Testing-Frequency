...
#Run the compois model on this data
analysis <- cprd$analysis("dh_hu")
for_upload <- for_upload %>% analysis$cached("hba1c_visits_dataset_cleaned_v12") %>% collect()



#Filter for individuals with data at 10yrs.
for_upload_10yrs <- for_upload %>% filter(stop == as.integer(stop) & hba1c_date_yrs <= 10) #472205

#Clear env
keep_var <- "for_upload_10yrs"
rm(list = setdiff(ls(), keep_var))

#Run the pre-existing depression model.
sampled_groups <- for_upload_10yrs %>%
  distinct(patid) %>%
  sample_n(50000)

sampled_dataset <- for_upload_10yrs %>% filter(patid %in% sampled_groups$patid) %>% 
  arrange(patid, hba1c_date_yrs)

sampled_dataset <- sampled_dataset %>% mutate(tds_2011 = scale(as.numeric(tds_2011)), 
                                   patid = as.factor(patid), 
                                   count = as.numeric(count),
                                   ethnicity_bin = as.factor(ethnicity_bin), 
                                   hba1c_date_yrs_scaled = as.numeric(hba1c_date_yrs), 
                                   gender = as.factor(gender), 
                                   incident_depression_timevar = as.factor(incident_depression_timevar),
                                   dm_diag_age_scaled = scale(as.numeric(dm_diag_age)), 
                                   incident_comorbidity_score_timevar_scaled = scale(as.numeric(incident_comorbidity_score_timevar)),
                                   preexisting_comorb_score_scaled = scale(as.numeric(preexisting_comorb_score)),
                                   yod_scaled = scale(year_of_diagnosis), pret2d_dep = as.factor(pret2d_dep))

library(glmmTMB)
library(splines)

test_model <- system.time(model_arc <- glmmTMB(count ~ ns(hba1c_date_yrs, 3) + (1 | patid) + 
                                                 gender + dm_diag_age_scaled + ethnicity_bin + tds_2011 + 
                                                 pret2d_dep + incident_comorbidity_score_timevar + 
                                                 preexisting_comorb_score + yod_scaled,
                                               data = sampled_dataset,   family = "compois", 
                                               control = glmmTMBControl(parallel = 9), verbose = T))


save(model_arc, file= "C:/Users/dhand/Desktop/vcf/GMF_all_covs_compois_140225.RData")
a <- get(load("C:/Users/dhand/Desktop/vcf/GMF_all_covs_compois_140225.RData"))

library(glmmTMB)
library(broom.mixed)
library(tidyverse)

extract_irr_compois <- function(model) {
  if (!inherits(model, "glmmTMB")) {
    stop("The input model must be a glmmTMB object.")
  }
  ...
  # Compute IRR (exponentiated coefficients)
  irr <- signif(exp(coefs[, "Estimate"]),3)
  
  # Compute 95% confidence intervals
  ci_lower <- signif(exp(coefs[, "Estimate"] - 1.96 * coefs[, "Std. Error"]),3)
  ci_upper <- signif(exp(coefs[, "Estimate"] + 1.96 * coefs[, "Std. Error"]),3)
  
  # Extract p-values
  p_values <- coefs[, "Pr(>|z|)"]
  p_values <- signif(p_values, 3)
  ...
  results <- data.frame(
    Exposure = rownames(coefs),
    IRR = irr,
    CI_95 = ci_95,
    P_Value = p_values
  )
  
  # Remove rows where Exposure contains "ns(" or "intercept"
  results <- results[!grepl("ns\\(|intercept", results$Exposure, ignore.case = TRUE), ]
  rownames(results) <- NULL
  
  return(results)
}

model_results <- extract_irr_compois(a)


summary <- for_upload_10yrs %>% distinct(patid, .keep_all = T)
mean(summary$dm_diag_age)
table(summary$gender)

summary_mort <- summary()
...
extract_hr_coxph <- function(models, model_names, variable) {
  ...
  results <- data.frame(Name = character(), HR = numeric(), CI_95 = character(), P_Value = numeric(), stringsAsFactors = FALSE)
  ...
  for (i in seq_along(models)) {
    ...
    if (!(variable %in% rownames(coefs))) {
      next  # Skip if the variable is not in the model
    }
    
    hr <- conf_int[variable, "exp(coef)"]
    ci_lower <- conf_int[variable, "lower .95"]
    ci_upper <- conf_int[variable, "upper .95"]
    p_value <- coefs[variable, "Pr(>|z|)"]
    
    # Create formatted 95% CI column
    ci_95 <- paste0("(", signif(ci_lower, 3), " - ", signif(ci_upper, 3), ")")
    
    # Append results to dataframe
    results <- rbind(results, data.frame(Name = name, HR = hr, CI_95 = ci_95, P_Value = p_value, stringsAsFactors = FALSE))
  }
  
  return(results)
}

for_upload <- convert_int_to_numeric(for_upload)


nopredep_df <- for_upload %>% filter(pret2d_dep == 0) %>% mutate(tds_2011 = scale(as.numeric(tds_2011)), 
                                                                 patid = as.factor(patid), 
                                                                 count = as.numeric(count),
                                                                 ethnicity_bin = as.factor(ethnicity_bin), 
                                                                 hba1c_date_yrs_scaled = as.numeric(hba1c_date_yrs), 
...
predep_df <- for_upload %>%  mutate(tds_2011 = scale(as.numeric(tds_2011)), 
                                                               patid = as.factor(patid), 
                                                               count = as.numeric(count),
                                                               ethnicity_bin = as.factor(ethnicity_bin), 
                                                               hba1c_date_yrs_scaled = as.numeric(hba1c_date_yrs), 
...
depsev_df <- for_upload %>% filter(pret2d_dep == 1) %>% mutate(tds_2011 = scale(as.numeric(tds_2011)), 
                                                              patid = as.factor(patid), 
                                                              count = as.numeric(count),
                                                              ethnicity_bin = as.factor(ethnicity_bin), 
                                                              hba1c_date_yrs_scaled = as.numeric(hba1c_date_yrs), 
...
#Calculate the total years of person follow-up and 
person_years <- for_upload %>% group_by(patid) %>% slice_tail()
person_years_calced <- sum(person_years$time_to_censor)
mortality_rate <- sum(person_years$event == 1)/person_years_calced
mortality_perc #17.8%
